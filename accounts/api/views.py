...
class AuthAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        qs = User.objects.filter(
            Q(email__iexact=username) | Q(email__iexact=username)
        ).distinct()
        if qs.count() == 1:
            user_obj = qs.first()
            if user_obj.check_password(password):
                ...
                response = jwt_response_payload_handler(
                    token, user=user, request=request
                )
...
#             # response = jwt_response_payload_handler(
#             #     token, user=user, request=request)
#             return Response({'detail': 'Thank you for registration. Please verify your email.'}, status=201)
